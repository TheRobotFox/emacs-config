...
** Custom
   ...
   (defun create-src-file ()
     ...
     (let* ((file (buffer-file-name))
      	 ...
      	 (inc (lambda () (insert "#include \"" (file-name-nondirectory file) "\"\n\n")))
      	 (create (lambda (new-ext &rest body-fns)
      		   (let ((new-file (file-name-with-extension file new-ext)))
      		     (find-file-other-window new-file)
      		     (if (file-exists-p new-file) (message "File already exits.")
      		       (mapcar (lambda (fn) (funcall fn)) body-fns))))))
       (cond
        ((equal ext "hpp")
         (funcall create "cpp" inc
      	       (let* ((root (treesit-buffer-root-node))
      		      (ns-decl (lambda (node)
      				 (or (treesit-node-eq root node)
      				     (equal (treesit-node-type node)
      					    "namespace_definition"))))
      		      (current-node (treesit-thing-at (point) ns-decl))
      		      (ns-name ""))
      		 
      		 (while (not (or (null current-node) (treesit-node-eq current-node root)))
      		   (progn
      		     (setq ns-name (concat
      				    (treesit-node-text
      				    ...
      				    (if (> (length ns-name) 0) "::" "")
      				    ns-name))
      		     (setq current-node (treesit-parent-until
      					 current-node ns-decl))))
      		 (lambda () (if (> (length ns-name) 0)
      				(insert "namespace " ns-name " {\n} // " ns-name))))))
        ((equal ext "h") (create "c" inc))
        (t (message "Language not recognized.")))))
#+end_src
** Doxygen
   #+begin_src emacs-lisp
  ...
  :vc (:url "https://github.com/pniedzielski/doxymacs.git"
            :rev :newest
            :lisp-dir "lisp/")
  :hook (c-ts-mode-common-hook . doxymacs-mode)
...
- Fancy Vars :: 
#+begin_src emacs-lisp
(defun cartesian-product-call (fn l1 l2)
  (mapcan (lambda (a)
...
(defvar my/fancy-vars
  (append (mapcar (lambda (character)
	  ...
	  (cartesian-product-call (lambda (character subscript)
...
- capture ::
  ...
  (use-package org
    :config
    (defun ha-file-for-class()
      (interactive)
      (let* ((dir "~/org/tu/")
  	   (class (completing-read "Modul: " (--filter (file-directory-p (file-name-concat dir it))
  						       (directory-files dir nil (rx (not "."))))))
  	   (class-dir (file-name-concat dir class))
  	   (ha-count (length (directory-files class-dir nil (rx "HA" digit))))
  	   (file (file-name-concat class-dir (format "HA%d.org" (1+ ha-count)))))
  	(set-buffer (find-file-noselect file))
  	(insert (format "#+TITLE: %s Hausaufgabe %d" class (1+ ha-count)))
  	(newline)))
   ...
   '(("t" "Todo" entry (file+headline "~/org/todos.org" "Todos")
     ...
     ("h" "Homework" plain (function ha-file-for-class) "")
     )))
  #+end_src
...
** Vue
   ...
   (defun my/lsp-volar-vue-filename-p (filename &optional _)
     "Check if FILENAME is a vue file."
     (string= (file-name-extension filename) "vue"))

   (defun my/lsp-volar-handle-tserver-request (workspace params)
     "Handle `tsserver/request' request from volar with PARAMS on WORKSPACE.

   see:
   - https://github.com/vuejs/language-tools/wiki/Neovim
   - https://github.com/vuejs/language-tools/discussions/5456"
     (pcase-let ((`[[,id ,command ,args]] params))
       (with-lsp-workspace (lsp-find-workspace 'ts-ls)
         (lsp-request-async
          "workspace/executeCommand" `(:command "typescript.tsserverRequest" :arguments [,command ,args])
          (lambda (resp)
            (with-lsp-workspace workspace
              (lsp-notify "tsserver/response" `[[,id ,(gethash "body" resp)]])))))))

   (defun my/lsp-volar-setup ()
     "Setup lsp-volar to support vue-ls v3."
     ;; Advice to activate ts-ls for .vue files
     (advice-add 'lsp-typescript-javascript-tsx-jsx-activate-p :after-until #'my/lsp-volar-vue-filename-p)

     ;; Add @vue/typescript-plugin to typescript-plugins
     (setq lsp-clients-typescript-plugins
           (seq-concatenate
            'vector
            (seq-remove (lambda (x) (string= (plist-get x :name) "@vue/typescript-plugin")) lsp-clients-typescript-plugins)
            '(( :name "@vue/typescript-plugin"
                :location "/usr/lib/node_modules/@vue/language-server"
                :languages ["vue"]))))

     (let ((volar (gethash 'vue-semantic-server lsp-clients)))
       ;; Make lsp-volar an add-on for ts-ls
       (setf (lsp--client-add-on? volar) t)
       ;; Handle tsserver/request from volar
       (setf (gethash "tsserver/request" (lsp--client-notification-handlers volar)) #'my/lsp-volar-handle-tserver-request)))
...
** MatLab
   ...
   (use-package matlab-mode
     :custom
     (lsp-clients-matlab-server "/home/new/packages/MATLAB-language-server/out/index.js"))
   #+end_src
** OCaml
   #+begin_src emacs-lisp
   (use-package tuareg)
